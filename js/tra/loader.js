...
async function loadLocalSpecifiedData() {
    const dateList = await (await fetch(`data/tra/specified/date.json`)).json();
    let specifiedData = [];
    for (const date of dateList) {
        const name = date["filename"];
        const data = await (await fetch(`data/tra/specified/${name}`)).json();
        specifiedData.push({
            "from": date["from"],
            "to": date["to"],
            "series": data
...
async function loadLocalOtherData() {
    const data = await (await fetch(`data/tra/other.json`)).json();
    return data;
}